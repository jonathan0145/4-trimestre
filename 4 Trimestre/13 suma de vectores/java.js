...
function sumarV(){
    ...
    if(v1.length > 5 || v2.length > 5){
        return pa.textContent = "digito mas de 5 numeros"; 
    }
    else if(v1.length < 5 || v2.length < 5){
        return pa.textContent = "digito menos de 5 numeros"; 
    }
    else if(v1.length == 5 && v1.length == 5){
        for (let i = 0; i < v1.length && i < v2.length; i++) {
            sumaV[i] = parseInt(v1[i]) + parseInt(v2[i]);
        }
        return pa.textContent = sumaV;  
    }
